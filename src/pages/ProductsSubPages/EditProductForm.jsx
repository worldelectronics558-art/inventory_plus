...
import { useNavigate, useParams } from 'react-router-dom'; // useParams to get product ID from URL
...
const EditProductForm = () => {
    ...
    // Find the specific product to edit
    const productToEdit = products.find(p => p.id === productId);

    // State for form data, errors, status, and lookup creation
    ...
    // Load product data into form state when productToEdit changes
    useEffect(() => {
        if (productToEdit) {
            setFormData({
        ...
        } else if (products.length > 0 && !productToEdit) { // Product not found in current list
             setStatus({ loading: false, error: 'Product not found.' });
        }
    }, [productToEdit, products]); // Depend on productToEdit and products list

    // Update default values if lookups change (e.g., after adding a new brand)
    // (This is less critical for editing, but kept for consistency if needed elsewhere)
    // useEffect(() => {
    //     setFormData(prev => ({
    //         ...prev,
    ...
    const handleSubmit = async (e) => {
        ...
        // SKU uniqueness check against *other* existing products (not the one being edited)
        const skuToCheck = formData.sku.trim();
        const existingProduct = products.find(p => p.sku.toLowerCase() === skuToCheck.toLowerCase() && p.id !== productId); // Exclude current product
        if (existingProduct) {
        ...
        try {
            ...
            await updateProduct(productId, productDataToSave); // Use context function
            setStatus({ loading: false, error: null });
            alert('Product updated successfully!'); // Or show success message differently
    ...
    const isDisabled = !isOnline || status.loading || !productToEdit; // Disable if product not loaded yet

    if (!productToEdit && status.error !== 'Product not found.') {
        // Still loading the product list or product is loading
            ...
            <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-6">
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <button
                        ...
                        className="btn btn-outline-secondary" // Use themed button
                    >
                        Back to Products
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            name="sku"
                            value={formData.sku}
                            onChange={handleChange} // Still call handleChange for potential error clearing
                            disabled={true} // SKU is typically not editable after creation
                            className="input-base bg-gray-100 text-gray-500 cursor-not-allowed" // Use themed input, show disabled state
                        />
