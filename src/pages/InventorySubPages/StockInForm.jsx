...
// --- STYLING FOR REACT-SELECT (retained for consistency) ---
...
const StockInForm = () => {
    ...
    const { stockIn } = useInventory(); // Get the new stockIn function
    ...
    const [items, setItems] = useState([{ 
        ...
        location: null, 
        quantity: 1, 
        isSerialized: false, // NEW: To track product type
        serials: [] // NEW: To hold serial numbers
    }]);
    ...
    const handleItemChange = (index, field, value) => {
        ...
        // When SKU changes, update isSerialized and reset serials
        if (field === 'sku') {
        ...
        // When quantity changes for a serialized item, adjust the serials array
        if (field === 'quantity' && currentItem.isSerialized) {
            const newQuantity = parseInt(value, 10) || 0;
            const currentSerials = currentItem.serials;
            const newSerials = Array.from({ length: newQuantity }, (_, i) => currentSerials[i] || '');
    ...
    const handleSubmit = async (e) => {
        ...
        for (const item of items) {
            if (!item.sku || !item.location || item.quantity <= 0) {
                alert('Please fill in all SKU, Location, and Quantity fields for all items.');
                return;
            }
            if (item.isSerialized) {
                if (item.serials.length !== item.quantity || item.serials.some(s => !s.trim())) {
                    alert(`Please enter all ${item.quantity} unique serial numbers for ${item.sku.label}.`);
                    return;
                }
                const serialSet = new Set(item.serials.map(s => s.trim()));
                if (serialSet.size !== item.quantity) {
                    alert(`Duplicate serial numbers found for ${item.sku.label}. Please ensure all are unique.`);
                    return;
                }
            }
        }
        ...
        try {
            const operationData = {
                ...
                items: items.map(item => ({
                    ...
                    // Pass trimmed serials, or an empty array if not serialized
                    serials: item.isSerialized ? item.serials.map(s => s.trim()) : [],
                    costPrice: null, // Placeholder for future implementation
                }))
            };
            ...
            alert(`Stock-in failed: ${error.message}`);
        } finally {
            ...
            <div className="min-h-screen bg-gray-100 p-4">
                <div className="max-w-7xl mx-auto bg-white rounded-lg shadow-md p-6">
                    <div className="flex justify-between items-center mb-6">
                        <h1 className="text-2xl font-bold">Stock In (Serialized)</h1>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                           <div>
                                ...
                                <input type="text" value={referenceNumber} onChange={(e) => setReferenceNumber(e.target.value)} placeholder="e.g., Invoice #, PO #" className="input-base" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium mb-1">Notes</label>
                                <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows="1" className="input-base" placeholder="Optional notes for this stock-in event" />
                            </div>
                        </div>
                                ...
                                <div key={index} className="border rounded-lg p-4 space-y-4 bg-gray-50">
                                        ...
                                        <div>
                                            <label className="block text-sm font-medium mb-1">Location *</label>
                                             <div className="flex items-center">
                                                <Select className="flex-grow" styles={customSelectStyles} options={locationOptions} value={item.location} onChange={v => handleItemChange(index, 'location', v)} placeholder="Select..." menuPortalTarget={document.body} />
                                                <button type="button" onClick={() => setAddLocationModalOpen(true)} className="btn-secondary p-2 ml-1">+</button>
                                            </div>
                                        </div>
                                        {/* Quantity */}
                                        <div>
                                            <label className="block text-sm font-medium mb-1">Quantity *</label>
                                            <input type="number" value={item.quantity} onChange={(e) => handleItemChange(index, 'quantity', parseInt(e.target.value) || 1)} min="1" className="input-base w-full" />
                                        </div>
                                        {/* Remove Button */}
                                    ...
                                    {item.isSerialized && item.quantity > 0 && (
                                        <div className="pt-4 mt-4 border-t">
                                            <h3 className="text-md font-semibold text-gray-700 mb-2">Enter Serial Numbers for {item.sku.label}</h3>
