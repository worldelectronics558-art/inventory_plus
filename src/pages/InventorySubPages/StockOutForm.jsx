...
const FORM_STATE_KEYS = { STOCK_OUT: 'formState_stockOut' };
// --- END: FORM STATE --- 
...
const StockOutForm = () => {
    ...
    const [items, setItems] = useState([{ sku: null, location: null, quantity: 1, reason: { value: 'sale', label: 'Sale' } }]);
    ...
    useEffect(() => {
        ...
        const loadSavedState = async () => {
            ...
            try {
                const savedState = await formStateStore.getItem(FORM_STATE_KEYS.STOCK_OUT);
    ...
    useEffect(() => {
        if (!isLoadingFormState) {
            ...
            const saveTimer = setTimeout(() => formStateStore.setItem(FORM_STATE_KEYS.STOCK_OUT, stateToSave), 500);
    ...
    const handleAddItem = () => {
        setItems([...items, { sku: null, location: null, quantity: 1, reason: { value: 'sale', label: 'Sale' }, currentQty: 0 }]);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        for (const item of items) {
            const currentQty = (stockLevels[item.sku.value] || {})[item.location.value] || 0;
            if (currentQty < item.quantity) {
                alert(`Insufficient stock for SKU ${item.sku.label} at location ${item.location.label}. Requested: ${item.quantity}, Available: ${currentQty}`);
                return;
            }
        }
    ...
    const handleReset = () => {
        ...
        setNotes('');
        setDocumentNumber('');
        setItems([{ sku: null, location: null, quantity: 1, reason: { value: 'sale', label: 'Sale' } }]);
        formStateStore.removeItem(FORM_STATE_KEYS.STOCK_OUT);
    };
    ...
    const handleExport = (format) => {
        ...
        const commonExport = (wb, doc) => {
            if (format === 'excel') XLSX.writeFile(wb, 'pending_stock_out_items.xlsx');
            else if (format === 'pdf') doc.save('pending_stock_out_items.pdf');
        }
        if (format === 'excel') {
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button type="submit" className="btn btn-danger">Commit Stock Out</button>
                            </div>
                        </div>
                    </form>
                </div>
