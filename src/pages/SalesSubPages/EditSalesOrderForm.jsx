...
const EditSalesOrderForm = () => {
    ...
    const { salesOrders, updateSalesOrder, isLoading, isMutationDisabled } = useSalesOrders();
    ...
    useEffect(() => {
        if (!isLoading && salesOrders.length > 0) {
            ...
            if (orderToEdit) {
                ...
                setFormState({
                    customerId: customerOptions.find(c => c.value === orderToEdit.customerId) || null,
                    orderDate: new Date(orderToEdit.orderDate.seconds * 1000).toISOString().slice(0, 10),
    ...
    }, [id, salesOrders, isLoading, customerOptions, productOptions]);
                ...
                <form onSubmit={handleSubmit}>
                    <div className="card p-6 mb-6">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                            ...
                            <div>
                                ...
                                <Select id="customerId" styles={customSelectStyles} options={customerOptions} value={formState.customerId} onChange={value => setFormState(prev => ({...prev, customerId: value}))} placeholder="Select a customer..." menuPortalTarget={document.body}/>
                            </div>
                            <div>
                                <label htmlFor="orderDate">Order Date *</label>
                                <input type="date" id="orderDate" value={formState.orderDate} onChange={(e) => setFormState(prev => ({...prev, orderDate: e.target.value}))} className="input-base" required />
                            </div>
                             <div>
                                <label htmlFor="documentNumber">Reference Number</label>
                                <input type="text" id="documentNumber" value={formState.documentNumber} onChange={(e) => setFormState(prev => ({...prev, documentNumber: e.target.value}))} className="input-base" placeholder="e.g., PO-123" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mt-8 pt-5 border-t border-gray-200 flex justify-end gap-3">
                        ...
                        <button type="submit" className="btn btn-primary" disabled={isMutationDisabled}>
                            ...
                            {isMutationDisabled ? 'Saving...' : 'Update Order'}
                        </button>
                    </div>
                </form>
            </div>
